...
void FindNeighbouringDistricts(GeoCollection &gc, const double eps){
  ...
  const double max_neighbour_pt_dist_squared = 500;
  ...
  for(unsigned int mpi=0;mpi<gc.g.size();mpi++){
    boost::geometry::for_each_point(gc.g.at(mpi).g, 
      [&](const Point2D &a){
        borders.emplace_back(a.x(),a.y());
        owner.push_back(mpi);
      }
    );
  }
  ...
  for(unsigned int i=0;i<borders.size();i++){
    ...
    for(const auto &sr: temp){
      if(owner[sr.first]==owner[i])
        continue;
      gc.g.at(owner.at(sr.first)).neighbours.insert(owner.at(i));
      gc.g.at(owner.at(i)).neighbours.insert(owner.at(sr.first));
    }
  }
}
...
void FindExteriorDistricts(
  ...
  std::cerr<<"Shrinking superunits and adding to boundary vector..."<<std::endl;
  //#pragma omp parallel for
  for(unsigned int mpi=0;mpi<superunits.g.size();mpi++){
    //Shrink the superunit by `shrink` units
    const auto shrunk_superunit = Buffer(superunits.g.at(mpi).g, -shrink);

    //const auto shrunk_superunit = superunits.at(mpi).clipper_paths;

    //Loop through all of the points of the superunit
    //#pragma omp critical
    boost::geometry::for_each_point(shrunk_superunit, 
      [&](const Point2D &a){
        borders.emplace_back(std::vector<double>{{a.x(),a.y()}});
        owner.push_back(mpi);
      }
    );
  }
  ...
  std::cerr<<"Determining parents..."<<std::endl;
  #pragma omp parallel for
  for(unsigned int subi=0;subi<subunits.g.size();subi++){
    //Alias the current subunit
    auto &sub = subunits.g.at(subi);
    ...
    boost::geometry::for_each_point(sub.g, 
      [&](const Point2D &pt){
...
void CalcParentOverlap(GeoCollection &subunits, GeoCollection &superunits){
  ...
  for(unsigned int i=0;i<subunits.g.size();i++){
    auto &sub = subunits.g.at(i);
    ...
    for(auto &p: sub.parents){
      double iarea = IntersectionArea(sub.g, superunits.g.at(p.first).g);
      p.second     = iarea/area;
    }
  ...
  for(unsigned int i=0;i<subunits.g.size();i++)
  for(auto &p: subunits.g.at(i).parents)
    superunits.g.at(p.first).children.emplace_back(i, p.second);
}
